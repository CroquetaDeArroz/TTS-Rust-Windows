...
const AUDIO_TMP: &str = if cfg!(target_os = "windows") {
    "C:\\Windows\\Temp\\tts_output.wav"
} else {
    "/tmp/tts_output.wav"
};
...
fn piper_dir() -> String {
    std::env::var("PIPER_DIR").unwrap_or_else(|_| {
        if cfg!(target_os = "windows") {
            ".\\files\\piper".to_string()
        } else {
            "./files/piper/piper".to_string()
        }
    })
}
...
fn generar_piper(texto: &str, modelo: &str) -> bool {
    ...
    {
        // En Windows: echo texto | piper.exe --model ... --output_file ...
        // Piper Windows viene como piper.exe en el directorio PIPER_DIR
        let piper_exe = format!("{}\\piper.exe", dir);
        let cmd = format!(
            "echo {} | \"{}\" --model \"{}\" --output_file \"{}\" 2>\"{}\"",
            texto_seguro, piper_exe, modelo, AUDIO_TMP,
            "C:\\Windows\\Temp\\piper_err.log"
        );
        match Command::new("cmd").args(["/C", &cmd])
            ...
            .spawn()
        {
            Ok(mut h) => {
                ...
                if !ok {
                    if let Ok(log) = std::fs::read_to_string("C:\\Windows\\Temp\\piper_err.log") {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        let cmd = format!(
            ...
            dir = dir, texto = texto_seguro, modelo = modelo,
        );
...
fn generar_coqui(texto: &str, modelo: &str, speaker_wav: &str) -> bool {
    ...
    // El binario tts de Coqui funciona igual en Windows y Linux
    // En Windows se instala como tts.exe vía pip
    let bin = if cfg!(target_os = "windows") { "tts.exe" } else { "tts" };
...
fn reproducir_wav(ruta: &str, _card: u8, volumen: f32) {
    #[cfg(target_os = "windows")]
    {
        // PowerShell: Media.SoundPlayer para WAV
        // El volumen se aplica multiplicando (0.0–2.0 → 0–100% con clamp)
        let vol_pct = ((volumen * 50.0) as u32).clamp(0, 100); // 1.0 = 50%, 2.0 = 100%
        let script = format!(
            r#"
$player = New-Object System.Media.SoundPlayer '{ruta}';
$player.PlaySync();
"#,
            ruta = ruta.replace('\'', "''")
        );
        // PowerShell no expone control de volumen en SoundPlayer directamente.
        // Usamos wscript para ajustar el volumen del sistema antes de reproducir.
        let vol_script = format!(
            r#"
$vol = {vol_pct};
$obj = New-Object -ComObject WScript.Shell;
# Ajustar volumen del sistema proporcionalmente
Add-Type -TypeDefinition '
using System.Runtime.InteropServices;
[Guid("5CDF2C82-841E-4546-9722-0CF74078229A"), InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
interface IAudioEndpointVolume {{ int f(); int ff(); int fff(); int SetMasterVolumeLevelScalar(float f, System.Guid g); }}
...
"#,
        ...
        // Versión simple y robusta: reproducir con SoundPlayer sin tocar volumen del sistema
        let script_simple = format!(
            "(New-Object System.Media.SoundPlayer '{}').PlaySync()",
            ruta.replace('\'', "''")
        );
        let ok = Command::new("powershell")
            .args(["-NoProfile", "-NonInteractive", "-Command", &script_simple])
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::null())
            .status()
            .map(|s| s.success())
        ...
        if !ok {
            // Fallback: Windows Media Player en modo silencioso
            let _ = Command::new("cmd")
                .args(["/C", "start", "/wait", "wmplayer", "/play", "/close", ruta])
                .status();
        }
    }
...
fn gtts_fallback(texto: &str, lang: &str) {
    let texto_seguro = texto.replace('\'', " ");
    let tmp_mp3 = if cfg!(target_os = "windows") {
        "C:\\Windows\\Temp\\tts_fb.mp3"
    } else {
        "/tmp/tts_fb.mp3"
    };

    let _ = Command::new("python3").arg("-c").arg(format!(
        "from gtts import gTTS; gTTS('{}', lang='{}').save('{}')",
        texto_seguro, lang, tmp_mp3
    )).output();

    #[cfg(target_os = "windows")]
    {
        // Usar Windows Media Player CLI para reproducir MP3
        let _ = Command::new("powershell")
            .args([
                "-NoProfile", "-NonInteractive", "-Command",
                &format!(
                    "$p=New-Object System.Windows.Media.MediaPlayer; $p.Open([uri]'{}'); $p.Play(); Start-Sleep -s 5; $p.Stop()",
                    tmp_mp3
                )
            ])
            .status();
    ...
    #[cfg(not(target_os = "windows"))]
    {
        let _ = Command::new("paplay").arg(tmp_mp3).status()
            .or_else(|_| Command::new("aplay").arg(tmp_mp3).status());
    }
}
...
fn main() {
    ...
    println!("   Lang    : {}", lang);
    println!("   OS      : {}", if cfg!(target_os = "windows") { "Windows" } else { "Linux" });
    ...
    for linea in lector.lines().map_while(Result::ok) {
        ...
        if let Some((usuario, texto)) = parsear_mensaje(&linea) {
            ...
            let msg = if cfg.anunciar_usuario {
                if *lang_arc == "en" { format!("{usuario} says {texto}") }
                else                 { format!("{usuario} dice {texto}") }
